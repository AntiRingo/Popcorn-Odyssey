...
const emptyMachine=()=>({loaded:false,buttered:false,cooking:0,ready:false,door:false,scooped:false});
...
function render(){
 ...
 $('instruction').textContent=box!==null?(box==='plain'?'拖动调料罐到纸盒撒料，或直接拖给客人作为原味。':'把纸盒拖给客人：符合口味全价，口味不符半价。'):m.ready?'出锅了！开门，点击拿起铲子，再点击机器盛取、点击纸盒装入。':m.cooking>0?'机器正在加热。可以提前脱粒，或切换到其他机器。':m.loaded?(m.buttered?'原料齐全，关闭机门并按机身电源开关。':'打开机门，把奶油罐拖进机器。'):'按住玉米来回摇晃脱粒，再将粒碗拖入已开门的机器。';renderCustomers();
}
...
function startMachine(m){if(!m.loaded||!m.buttered||m.cooking>0||m.ready)return false;m.door=false;m.cooking=6;return true;}
...
function transferPopcorn(index,slot){const m=machines[index];if(!m||!m.ready||m.scooped||boxes[slot]!==null)return false;boxes[slot]='plain';machines[index]=emptyMachine();return true;}
